  ...
  task :save_videos => :environment do
    ...
    keywords.each do |keyword|
      ...
      results.items.each do |item|
        ...
        if video.snippet.title.include?(game_name) || video.snippet.description.include?(game_name) && video.snippet.title.include?(keyword.name) || video.snippet.description.include?(keyword.name)
          # 動画情報をデータベースに保存
          ...
          unless Video.exists?(video_id: video_id)
            video_record.save
          end
        end
      end
